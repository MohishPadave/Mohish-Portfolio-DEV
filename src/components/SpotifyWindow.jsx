...
const SpotifyWindow = ({ onClose, isMinimized, onMinimize }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center pb-32 px-4 pointer-events-none transition-opacity duration-300 ${isMinimized ? 'opacity-0 pointer-events-none' : ''}`}>
      <div
        className={`relative bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 rounded-xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden flex flex-col border border-white/10 pointer-events-auto ${isMaximized
