...
const FinderWindow = ({ onClose, onOpenPreview }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center pb-12 sm:pb-16 px-2 sm:px-4 pointer-events-none">
      <div
        className={`relative bg-[#1e1e1e] rounded-lg sm:rounded-xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden flex flex-col border border-gray-700 pointer-events-auto ${
          isMaximized 
            ...
            {!openedFolder ? (
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 sm:gap-4 md:gap-6">
                {folders.map((folder) => (
                  <div
                    ...
                    className={`flex flex-col items-center gap-1 sm:gap-2 cursor-pointer group p-1.5 sm:p-2 rounded-lg transition-colors ${
                      selectedFolder?.id === folder.id ? 'bg-blue-600/20' : 'hover:bg-gray-700/30'
                    }`}
                  >
                    {/* Folder Icon */}
                    ...
                    {/* Folder Name */}
                    <span className={`text-[10px] sm:text-xs text-center transition-colors truncate w-full ${
                      selectedFolder?.id === folder.id ? 'text-white font-medium' : 'text-gray-300 group-hover:text-white'
                    }`}>
                      {folder.name}
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 sm:gap-4 md:gap-6">
                <div
                  ...
                  className={`flex flex-col items-center gap-1 sm:gap-2 cursor-pointer group p-1.5 sm:p-2 rounded-lg transition-colors ${
                    selectedFile === 'resume' ? 'bg-blue-600/20' : 'hover:bg-gray-700/30'
                  }`}
                >
                  {/* PDF Icon */}
                  <div className="relative w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 flex items-center justify-center">
                  ...
                  {/* PDF Name */}
                  <span className={`text-[10px] sm:text-xs text-center transition-colors truncate w-full ${
                    selectedFile === 'resume' ? 'text-white font-medium' : 'text-gray-300 group-hover:text-white'
                  }`}>
                    Resume.pdf
                  </span>
                </div>
