...
const NotesApp = ({ onClose, onMinimize, isMinimized }) => {
      ...
      case 'About Me':
        return (
          <div className="p-4 sm:p-6 md:p-8 lg:p-12 w-full flex justify-center">
            <div className={`w-full transition-all duration-500 ${isMaximized ? 'max-w-none px-4 sm:px-12' : 'max-w-4xl'}`}>
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 pb-24 sm:pb-32 transition-opacity duration-300 ${isMinimized ? 'opacity-0 pointer-events-none' : ''}`}>
      <div className="absolute inset-0 bg-black/20" onClick={onClose}></div>

      <div className={`relative bg-white rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden flex transition-all duration-300 ${isMaximized
          ...
          <div className="space-y-0.5 sm:space-y-1">
            {tabs.map((tab) => (
              <button
                ...
                className={`w-full text-left px-2 sm:px-4 md:px-6 py-1.5 sm:py-2 transition-colors text-[10px] sm:text-xs md:text-sm truncate ${activeTab === tab.id
